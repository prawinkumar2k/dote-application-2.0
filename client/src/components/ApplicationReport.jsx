      ...
      <div className="bg-white p-8 print:p-6 print:max-w-full">
        ...
        <div className="mb-8 pb-8 border-b-2 border-slate-300">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-500 text-sm mt-2">DOTE - Department of Technical Education</p>
            {s.application_no && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div className="lg:col-span-3">
            <h2 className="text-xl font-bold text-slate-900 mb-6 pb-3 border-b-2 border-blue-500">Personal Information</h2>
        ...
        <div className="mb-8 p-6 bg-slate-50 rounded-lg border border-slate-200">
          ...
          <div className="mt-4 pt-4 border-t border-slate-300">
            <p className="text-sm font-bold text-slate-700 mb-2">Communication Address:</p>
            <p className="text-sm text-slate-600">{s.communication_address || '—'}</p>
          </div>
        </div>

        {/* Parental Information */}
        <div className="mb-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
          <h2 className="text-lg font-bold text-slate-900 mb-4">Parental Information</h2>
          <div className="grid grid-cols-2 gap-6">
        ...
        {m && (
          <div className="mb-8">
            ...
            <div className="mb-6 p-4 bg-emerald-50 rounded-lg border border-emerald-200">
              <h3 className="font-bold text-emerald-900">Qualifying Board: <span className="text-emerald-600">{m.qualifying_board}</span></h3>
              <p className="text-sm text-emerald-800 mt-1">Year: {m.year_of_passing}</p>
            </div>
            ...
            {subjects.length > 0 && (
              <div className="mb-6">
                <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                  <BookOpen size={18} /> HSC Academic Performance
                </h2>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-blue-100">
                        <th className="border border-slate-300 p-3 text-left font-bold text-slate-900">Subject</th>
                        <th className="border border-slate-300 p-3 text-center font-bold text-slate-900">Obtained</th>
                        <th className="border border-slate-300 p-3 text-center font-bold text-slate-900">Max Marks</th>
                        <th className="border border-slate-300 p-3 text-center font-bold text-slate-900">Percentage</th>
                      </tr>
                    </thead>
                    <tbody>
                      {subjects.map((sub, idx) => (
                        <tr key={idx} className="even:bg-slate-50 hover:bg-blue-50">
                          <td className="border border-slate-300 p-3 text-slate-700">{sub.name}</td>
                          <td className="border border-slate-300 p-3 text-center text-slate-700 font-bold">{sub.obtained}</td>
                          <td className="border border-slate-300 p-3 text-center text-slate-700">{sub.max}</td>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                    <p className="text-sm text-emerald-700 font-bold">HSC Percentage</p>
                    <p className="text-2xl font-bold text-emerald-600">{m.hsc_percentage || '—'}%</p>
                  </div>
                  <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                    <p className="text-sm text-purple-700 font-bold">HSC Cutoff Score</p>
                    <p className="text-2xl font-bold text-purple-600">{m.hsc_cutoff || '—'}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {sslcSubjects.length > 0 && (
              <div className="mb-6">
                <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                  <Award size={18} /> SSLC Academic Performance
                </h2>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm border-collapse">
                    <thead>
                    ...
                    <tbody>
                      {sslcSubjects.map((sub, idx) => (
                        <tr key={idx} className="even:bg-slate-50 hover:bg-orange-50">
                          <td className="border border-slate-300 p-3 text-slate-700">{sub.name}</td>
                          <td className="border border-slate-300 p-3 text-center text-slate-700 font-bold">{sub.obtained}</td>
                          <td className="border border-slate-300 p-3 text-center text-slate-700">{sub.max}</td>
                          <td className="border border-slate-300 p-3 text-center text-slate-700">
                ...
                <div className="mt-4">
                  <div className="p-4 bg-orange-50 rounded-lg border border-orange-200 w-fit">
                    <p className="text-sm text-orange-700 font-bold">SSLC Percentage</p>
                    <p className="text-2xl font-bold text-orange-600">{m.sslc_percentage || '—'}%</p>
                  </div>
                </div>
              </div>
            )}
        ...
        {/* Footer */}
        <div className="mt-12 pt-8 border-t-2 border-slate-300 text-center text-sm text-slate-600">
